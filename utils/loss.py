...
def calculate_gan_loss(discriminator, output, target):
...
def sift_loss_l1(img_true,img_fake):
    img_true=kornia.color.rgb_to_grayscale(img_true)
    img_fake=kornia.color.rgb_to_grayscale(img_fake)
    discriptor = kornia.feature.DenseSIFTDescriptor(rootsift=False)
    sift_true=discriptor(img_true)
    sift_fake=discriptor(img_fake)
    return torch.sum(torch.abs(sift_true-sift_fake))/(sift_true.shape[2]*sift_true.shape[3])