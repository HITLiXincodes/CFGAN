...
def _compute_statistics_of_path(path, model, batch_size, dims, cuda, flag=False):
    npz_file = os.path.join(path, 'statistics.npz')
    if os.path.exists(npz_file) and False:
        f = np.load(npz_file)
        m, s = f['mu'][:], f['sigma'][:]
        f.close()
    else:
        if flag:
            files = np.load('/home/cvking/SS/flist/celebahq_test.npy')
            files = files[:2000]
            ...
            files = files[:99]
            imgs = np.array([cv2.imread(str(path + fn)).astype(np.float32) for fn in files])
...
def fid(paths):
    fid_value = calculate_fid_given_paths(paths, 1, True, 2048)
    return fid_value


if __name__ == '__main__':
    # downgrade scipy from 1.4.1 to 1.3.3
    model = InceptionV3([3])

    args = parser.parse_args()
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu

    path1 = 'path_to_the_Generated_Results'
    path2 = 'path_to_the_Ground_Truth'
